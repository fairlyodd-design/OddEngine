...
function buildInternalRenderJobJson(
  title: string,
  ...
  const storyboard = latestAssetOfKind(assets, ["storyboard", "shotList", "videoTreatment"]);
  const scenes = parseStoryboardScenes(storyboard?.content || "");
  return JSON.stringify(
    {
      type: "oddengine.internalRenderJob",
      version: "1.0",
      ...
      assets: {
        writing: assets.filter((a) => ["story", "song", "featureOutline", "episodeGuide"].includes(a.kind)).map((a) => a.title),
        visual: assets.filter((a) => ["storyboard", "videoTreatment", "artPromptPack", "cartoonBible", "shotList"].includes(a.kind)).map((a) => a.title),
        production: assets.filter((a) => ["productionPack", "productionRunbook", "castingPack", "renderHandoff", "screeningPacket", "renderJob"].includes(a.kind)).map((a) => a.title),
      },
      notes: [
        "This is an internal OddEngine render-job package.",
        "It is intended for a future local worker or an external render bridge.",
        "OddEngine is preparing the project and preview lane here, not rendering a full film natively yet.",
      ],
    },
    null,
    2
...
function buildSceneRenderQueue(title: string, visualStyle: VisualStyle, assets: StudioAsset[]) {
  const storyboard = latestAssetOfKind(assets, ["storyboard", "shotList", "videoTreatment"]);
  const scenes = parseStoryboardScenes(storyboard?.content || "");
  const rows = scenes.length
    ? scenes
    : [{ title: "Scene 1", summary: "Opening image / hook", camera: "Wide establish", purpose: "Hook" }];
  ...
  return [
    ...
    ``,
    ...rows.flatMap((scene, idx) => [
      `## Queue ${idx + 1} — ${scene.title}`,
      `- Summary: ${scene.summary}`,
      `- Camera: ${scene.camera}`,
      `- Purpose: ${scene.purpose}`,
      `- Deliverables: keyframe still, motion pass, transition safety shot`,
      `- Continuity: keep character / palette / prop consistency with previous scene`,
      ``,
    ]),
  ].join("\n");
}

function buildWatchDeckManifest(
  title: string,
  productionType: ProductionType,
  ...
  return [
    ...
    ``,
    `## Watch-ready package`,
    `- [ ] Scene queue completed`,
    `- [ ] Voice / music assets aligned`,
    `- [ ] Continuity board approved`,
    `- [ ] Final assembly manifest locked`,
    `- [ ] Master render imported into Writers preview`,
    ``,
    `## Viewer-facing outputs`,
    `- Main feature / short / music video render`,
    `- Trailer / teaser cut`,
    `- Poster stills`,
    `- Promo stills / thumbnails`,
...
export default function Books({ onNavigate }: { onNavigate: (panelId: string) => void }) {
  ...
  const studioComposerRef = useRef<HTMLDivElement | null>(null);
  const studioPipelineRef = useRef<HTMLDivElement | null>(null);
  const studioProducerRef = useRef<HTMLDivElement | null>(null);
  const studioRenderRef = useRef<HTMLDivElement | null>(null);
...
Generate an art prompt pack from a saved asset to populate this export.`;
  ...
  const studioRoomCards = useMemo(() => {
    const hasSeed = Boolean((studioPrompt || "").trim() || active?.logline || active?.title);
    const hasMusic = studioAssets.some((asset) => asset.kind === "song");
    const hasRender = studioAssets.some((asset) => ["renderHandoff", "renderJob", "screeningPacket"].includes(asset.kind)) || Boolean(renderPreviewUrl);
    return [
      {
        key: "write",
        eyebrow: "STUDIO HOME",
        title: "Writing Room",
        status: hasSeed ? "Seeded" : "Needs seed",
        blurb: "Shape the core idea, story beats, and first draft from one writing-first room.",
      },
      {
        key: "direct",
        eyebrow: "CREATIVE FLOW",
        title: "Director Room",
        status: greenlightModel.hasVisual ? "Visual lane active" : "Storyboard next",
        blurb: "Turn the project into storyboard logic, camera movement, scenes, and shot progression.",
      },
      {
        key: "music",
        eyebrow: "SOUND + FEEL",
        title: "Music Lab",
        status: hasMusic ? "Scoring in play" : "Cue sheet pending",
        blurb: "Lock emotional tone, song ideas, soundtrack cues, and voice + music direction.",
      },
      {
        key: "render",
        eyebrow: "DELIVERY PIPELINE",
        title: "Render Lab",
        status: hasRender ? "Render-ready" : "Prep render handoff",
        blurb: "Package prompts, scene batches, and render metadata so the project can actually ship.",
      },
      {
        key: "ship",
        eyebrow: "GO TO MARKET",
        title: "Producer Ops",
        status: greenlightModel.hasProduction ? "Ops assets ready" : "Need ops pack",
        blurb: "Greenlight the project, prep launch docs, and move from creative idea to finished product.",
      },
    ] as const;
  }, [studioPrompt, active?.logline, active?.title, studioAssets, renderPreviewUrl, greenlightModel.hasVisual, greenlightModel.hasProduction]);

  const focusStudioRoom = (room: "write" | "direct" | "music" | "render" | "ship") => {
    if (room === "write") {
      setWriterMode("story");
      studioComposerRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
      return;
    }
    if (room === "direct") {
      studioPipelineRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
      return;
    }
    if (room === "music") {
      setWriterMode("song");
    ...
    if (room === "render") {
      studioRenderRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
      return;
    }
    studioProducerRef.current?.scrollIntoView({ behavior: "smooth", block: "start" });
  };
  ...
  return (
    <div className="panelRoot writersStudioRoot">
      <PanelHeader
        title="🎬 FairlyOdd Studio"
        subtitle="Idea → write → direct → score → render → ship from one creative engine."
        panelId="Books"
      ...
      <div className="creativeHeroBand booksHeroBand">
        <div className="creativeHeroCard">
          ...
          <div className="creativeHeroTitle">Studio Home</div>
          <div className="creativeHeroSub">One creative heartbeat for writing, directing, scoring, render prep, and producer ops.</div>
        </div>
        <div className="creativeMetricStrip">
          <div className="creativeMetricCard"><div className="small shellEyebrow">PROJECTS</div><div className="h">{books.length}</div></div>
          <div className="creativeMetricCard"><div className="small shellEyebrow">STUDIO ASSETS</div><div className="h">{studioAssets.length}</div></div>
          <div className="creativeMetricCard"><div className="small shellEyebrow">GREENLIGHT</div><div className="h">{greenlightModel.readiness}</div></div>
          <div className="creativeMetricCard"><div className="small shellEyebrow">RENDER LANE</div><div className="h">{renderProvider}</div></div>
        </div>
      </div>
      ...
      <div className="writersGrid">
        ...
        <div className="writersLeft">
          ...
          <CardFrame title="Studio Flow" subtitle="Move through the creative pipeline without losing the thread." storageKey="writers:tools" className="softCard" defaultCollapsed={false}>
            <div className="row wrap">
              <button className="tabBtn" onClick={() => focusStudioRoom("write")}>✍️ Writing Room</button>
              <button className="tabBtn" onClick={() => focusStudioRoom("direct")}>🎬 Director Room</button>
              <button className="tabBtn" onClick={() => focusStudioRoom("music")}>🎵 Music Lab</button>
              <button className="tabBtn" onClick={() => focusStudioRoom("render")}>🖥️ Render Lab</button>
              <button className="tabBtn" onClick={() => focusStudioRoom("ship")}>📦 Producer Ops</button>
            </div>
            ...
            <div className="note">
              Studio should feel like one pipeline: seed the idea, write it, direct it, score it, prep the render, then package it to ship.
            </div>
          </CardFrame>
        </div>
        ...
        <div className="writersCenter">
          <div className="card softCard">
            <div className="cluster wrap spread">
              <div>
                <div className="h">Writing Room</div>
                <div className="sub">Draft, revise, and keep the project moving without losing the bigger studio flow.</div>
              </div>
              <div className="tabs">
        ...
        <div className="writersRight">
          <div className="card softCard writersAICard">
            <div className="cluster wrap spread">
              <div>
                <div className="h">Studio Home</div>
                <div className="sub">Move the project from idea → write → direct → score → render → ship without leaving the creative lane.</div>
              </div>
              <div className="row wrap">
            ...
            <div className="studioHeartbeatStrip mt-4">
              <div className="studioHeartbeatCard">
                <div className="small shellEyebrow">IDEA</div>
                <div className="h">{studioPrompt?.trim() ? "Seed locked" : "Need seed"}</div>
                <div className="small">Prompt, title, and project intent live here first.</div>
              </div>
              <div className="studioHeartbeatCard">
                <div className="small shellEyebrow">WRITE</div>
                <div className="h">{greenlightModel.hasWriting ? "Writing active" : "Draft next"}</div>
                <div className="small">Stories, songs, outlines, and the first real shape of the project.</div>
              </div>
              <div className="studioHeartbeatCard">
                <div className="small shellEyebrow">DIRECT</div>
                <div className="h">{greenlightModel.hasVisual ? "Visual lane active" : "Storyboard next"}</div>
                <div className="small">Treatment, storyboard, world rules, and camera logic.</div>
              </div>
              <div className="studioHeartbeatCard">
                <div className="small shellEyebrow">SHIP</div>
                <div className="h">{greenlightModel.hasProduction ? "Producer ready" : "Ops pack next"}</div>
                <div className="small">Runbooks, release docs, render prep, and launch assets.</div>
              </div>
            </div>

            <div className="studioRoomGrid mt-4">
              {studioRoomCards.map((room) => (
                <button key={room.key} className="studioRoomCard" onClick={() => focusStudioRoom(room.key)}>
                  <div className="small shellEyebrow">{room.eyebrow}</div>
                  <div className="h">{room.title}</div>
                  <div className="studioRoomStatus">{room.status}</div>
                  <div className="small">{room.blurb}</div>
                </button>
              ))}
            </div>

            <div className="writersStudioComposer mt-4" ref={studioComposerRef}>
              <div className="small shellEyebrow">WRITING ROOM / CREATIVE SEED</div>
              <textarea
                className="input"
                style={{ minHeight: 110 }}
                value={studioPrompt}
            ...
            <div className="writersPipelineBoard mt-4" ref={studioPipelineRef}>
            ...
            <div className="writersGreenlightBoard mt-4" ref={studioProducerRef}>
              <div className="cluster wrap spread">
                <div>
                  <div className="h">Producer Ops</div>
            ...
            <div className="writersReleaseEngine mt-4">
              <div className="cluster wrap spread">
                <div>
                  <div className="h">Ship / Release</div>
            ...
            <div className="writersRenderPrep mt-4" ref={studioRenderRef}>
              <div className="cluster wrap spread">
                <div>
                  <div className="h">Render Lab</div>
