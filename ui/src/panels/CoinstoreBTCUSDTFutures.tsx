...
import { buildHeikinAshiSvg, buildPhoenixCoach } from "../lib/coinstoreFuturesRuntime";
...
function emptyCandleMap(mark = 70670): Record<TimeframeId, CoinstoreCandle[]> {
  return {
    "1m": seedMockCandles(mark - 20, "1m"),
    "3m": seedMockCandles(mark - 10, "3m"),
    "5m": seedMockCandles(mark, "5m"),
    "15m": seedMockCandles(mark + 10, "15m"),
  };
}
...
export default function CoinstoreBTCUSDTFutures() {
  ...
  const [candlesByTf, setCandlesByTf] = useState<Record<TimeframeId, CoinstoreCandle[]>>(emptyCandleMap(FALLBACK_SNAPSHOT.markPrice));
  ...
  useEffect(() => {
    setPaperTrades((current) => {
      const updated = current.map((trade) => updateOpenTradeMark(trade, snapshot.markPrice));
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        ...
        const btc = configJson?.data?.contracts?.find((entry: any) => entry?.name === "BTCUSDT");
        ...
        if (!cancelled) {
          ...
          setCandlesByTf(emptyCandleMap(FALLBACK_SNAPSHOT.markPrice));
          setStatus("fallback");
        }
      }
    }
      ...
      <div className="card softCard" style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
          <div>
            ...
            <div className="sub">
              Stable Heikin Ashi chart, macro-style setup read, calm coach cues, and paper validation lane.
            </div>
          </div>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <span style={{ borderRadius: 999, padding: "5px 8px", fontSize: 11, border: "1px solid rgba(255,255,255,0.08)", ...toneStyle(biasTone(phoenixPlan.primaryBias)) }}>
      ...
      <div style={adaptiveTwoCol}>
            ...
            <div style={{ marginTop: 12 }}>
              <svg viewBox="0 0 780 240" style={{ width: "100%", height: 240, borderRadius: 18 }} role="img" aria-label="BTCUSDT Phoenix sniper Heikin Ashi chart">
                ...
                {chartSvg.bodies.map((body, index) => (
                  <g key={index}>
                    <line x1={body.x} x2={body.x} y1={body.wickHighY} y2={body.wickLowY} stroke={body.bullish ? "rgba(94,201,111,0.95)" : "rgba(255,111,97,0.95)"} strokeWidth="1.4" />
                    <rect x={body.x - body.width / 2} y={body.bodyY} width={body.width} height={body.bodyH} rx="1.5" fill={body.bullish ? "rgba(94,201,111,0.92)" : "rgba(255,111,97,0.92)"} />
                  </g>
                ))}
              </svg>
            </div>
          ...
          <div className="card softCard">
            ...
            <div className="stack" style={{ marginTop: 12 }}>
              {phoenixPlan.executionNotes.map((note) => (
                <div style={row} key={note}><span className="muted">{note}</span></div>
              ))}
            </div>
          </div>
        </div>
        ...
        <div className="stack">
          <div className="card softCard">
            ...
            <div className="sub" style={{ marginTop: 4 }}>Stable conversation cues instead of runtime noise.</div>
            <div className="stack" style={{ marginTop: 12 }}>
              {coachCues.map((cue) => (
          ...
          <div className="card softCard">
            ...
            <div className="stack" style={{ marginTop: 12 }}>
              {macroIntel.bullets.map((bullet) => (
                <div style={row} key={bullet}><span className="muted">{bullet}</span></div>
              ))}
            </div>
            <div className="stack" style={{ marginTop: 12 }}>
              {macroIntel.warnings.map((warning) => (
                <div style={{ ...row, borderColor: toneStyle("warn").borderColor, background: toneStyle("warn").background }} key={warning}><span className="muted">{warning}</span></div>
              ))}
            </div>
          </div>
          ...
          <div className="card softCard">
            ...
            <div className="sub" style={{ marginTop: 4 }}>Sim the sniper before risking real ammo.</div>
            <div style={{ ...adaptiveStats, marginTop: 12 }}>
              <div className="stack">
          ...
          <div className="card softCard">
            ...
            <div className="stack" style={{ marginTop: 12 }}>
              {paperTrades.length ? paperTrades.map((trade) => (
                <div style={row} key={trade.id}>
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div className="muted">{trade.timeframeFocus} · entry {trade.entry.toFixed(1)} · stop {trade.stop.toFixed(1)} · size ${trade.sizeUsd} · {trade.leverage}x</div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "flex-end" }}>
                    <span style={{ borderRadius: 999, padding: "5px 8px", fontSize: 11, border: "1px solid rgba(255,255,255,0.08)", ...toneStyle(trade.status === "open" ? "warn" : trade.pnlUsd > 0 ? "good" : "bad") }}>
                      {trade.status === "open" ? `Open · ${trade.pnlUsd >= 0 ? "+" : ""}${trade.pnlUsd.toFixed(2)}` : `${trade.closeReason || "Closed"} · ${trade.pnlUsd >= 0 ? "+" : ""}${trade.pnlUsd.toFixed(2)}`}
                    </span>
                    {trade.status === "open" ? (
              ...
              )) : <div style={row}><span className="muted">No paper trades yet.</span></div>}
            </div>
          </div>
          ...
          <div className="card softCard">
            ...
            <div className="sub" style={{ marginTop: 4 }}>Keep keys local only. This lane is for your machine, not the repo.</div>
            <div className="stack" style={{ marginTop: 12 }}>
