...
export async function fetchGroceryProxyDeals(baseUrl: string, query: string, stores: string[] = []): Promise<FeedItem[]> {
  ...
  return rows.slice(0, 16).map((row) => ({
    ...
    link: row.link || root,
    source: row.source || row.store || "Local grocery proxy",
    publishedAt: row.publishedAt || data?.updatedAt || "",
    summary: row.summary || (row.score != null ? `Proxy score ${row.score}` : "Pulled from local grocery proxy."),
