  ...
  useEffect(() => {
    ...
    const attach = () => {
      ...
      const cards = Array.from(root.querySelectorAll<HTMLElement>(".card"))
        ...
        // Trading is dense and data-heavy; only attach layout chrome to the main dockable cards.
        .filter((c) => panelId !== "Trading" || tradingAllowedIds.has(c.id));
      ...
      cards.forEach((card, idx) => {
        ...
        const dragHandle = headerToggleEl || card;
