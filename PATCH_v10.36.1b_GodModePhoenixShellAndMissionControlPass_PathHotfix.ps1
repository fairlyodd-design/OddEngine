...
foreach ($candidate in $candidateRoots) {
  $tryPath = Join-Path $candidate "ui\src\App.tsx"
...
if (!(Test-Path $appPath)) {
  $checked = ($candidateRoots | ForEach-Object { Join-Path $_ "ui\src\App.tsx" }) -join "; "
  throw "Could not find ui\\src\\App.tsx. Checked: $checked"
}

Write-Host "Using OddEngine root: $root"
...
Write-Host "Patched App.tsx successfully at $appPath"
