...
cls
echo ========================================
echo   FAIRLYODD OS + HOMIE LAUNCHER v10.36.81
echo ========================================
echo.
echo Starts render, music, Homie voice bridge, then desktop UI.
echo.
echo NOTE:
echo Keep the service windows open while using FairlyOdd OS.
echo.
...
echo Cleaning old node/electron/browser helper processes...
taskkill /IM node.exe /F >nul 2>&1
taskkill /IM electron.exe /F >nul 2>&1

echo.
echo Starting render backend (%RENDER_PORT%)...
...
echo.
echo Starting music bridge (%MUSIC_PORT%)...
...
echo.
echo Starting FairlyOdd desktop...
...
echo ========================================
echo.
echo Tip: Keep all launched windows open.
pause
exit /b 0
