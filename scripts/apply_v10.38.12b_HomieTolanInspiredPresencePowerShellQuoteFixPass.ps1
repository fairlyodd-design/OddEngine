...
Write-Host "[v10.38.12b] Applying quote-safe Homie avatar/presence polish..." -ForegroundColor Cyan
...
if (Test-Path $version) {
  ...
  $ver = [regex]::Replace($ver, 'export const APP_VERSION = ".*?";', 'export const APP_VERSION = "10.38.12b";')
  if ($ver -notmatch 'HOMIE_TOLAN_INSPIRED_PRESENCE_QUOTE_FIX_PASS') {
    $ver = $ver.TrimEnd() + "`r`n" + 'export const HOMIE_TOLAN_INSPIRED_PRESENCE_QUOTE_FIX_PASS = "v10.38.12b_HomieTolanInspiredPresencePowerShellQuoteFixPass";' + "`r`n"
  }
  [System.IO.File]::WriteAllText($version, $ver, $utf8NoBom)
}
Write-Host "[v10.38.12b] Applied. Quote-safe visual + copy polish." -ForegroundColor Green
Write-Host "Next:"
