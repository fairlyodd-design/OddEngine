...
Write-Host "[v10.38.8e] Applying HomieBuddy source avatar slot fix..." -ForegroundColor Cyan
...
$beforeRive = $tsx
$tsx = [regex]::Replace($tsx, '<RiveHomie\b[\s\S]*?\/>', '<HomieBuddyHumanAvatar />')
$replacedRive = ($beforeRive -ne $tsx)

if (-not $replacedRive -and $tsx -notmatch '<HomieBuddyHumanAvatar />') {
...
if ($tsx -notmatch 'homieBuddyHumanSyncedNote') {
  $anchor = '<div className="assistantSectionTitle">A calmer Homie lane</div>'
  if (-not $tsx.Contains($anchor)) { $anchor = '<div className="assistantSectionTitle">Human Homie companion lane</div>' }
  if ($tsx.Contains($anchor)) {
    $note = '<div className="homieBuddyHumanSyncedNote"><b>Human Homie companion lane</b><p>Same warm face as main Homie: cap, glasses, beard, kind eyes, and one calm next step.</p></div>' + "`r`n" + '              '
    $tsx = $tsx.Replace($anchor, $note + $anchor)
  }
}
...
$cssPayload = Join-Path $payload "HOMIE_COMPANION_SOURCE_AVATAR_FIX.css"
...
$start = "/* ===== v10.38.8e Homie Companion Source Trace + Avatar Slot Fix ===== */"
$end = "/* ===== v10.38.8e Homie Companion Source Trace + Avatar Slot Fix END ===== */"
...
if (Test-Path $version) {
  ...
  $ver = [regex]::Replace($ver, 'export const APP_VERSION = ".*?";', 'export const APP_VERSION = "10.38.8e";')
  if ($ver -notmatch 'HOMIE_COMPANION_SOURCE_AVATAR_FIX_PASS') {
    $ver = $ver.TrimEnd() + "`r`n" + 'export const HOMIE_COMPANION_SOURCE_AVATAR_FIX_PASS = "v10.38.8e_HomieCompanionSourceTraceAndAvatarSlotFixPass";' + "`r`n"
  }
  [System.IO.File]::WriteAllText($version, $ver, $utf8NoBom)
}

Write-Host "[v10.38.8e] Applied." -ForegroundColor Green
if ($replacedRive) {
  Write-Host "[v10.38.8e] Replaced RiveHomie avatar instance(s) in HomieBuddy.tsx." -ForegroundColor Green
} else {
  Write-Host "[v10.38.8e] Used companion lane title fallback." -ForegroundColor Yellow
}
Write-Host "Next:"
